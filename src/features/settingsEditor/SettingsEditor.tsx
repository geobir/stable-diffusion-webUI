...
const isImg2Img = (settings: X2ImgSettings) => {
...
const stringToNumbers = (s:string) => {
...
export default function SettingsEditor() {
    ...
    const typeCounts = useTheStore(s => {
        ...
        ids.forEach(id => {
            const settings = s.nextItems.byId[id];
            if (isX2Img(settings)) {
                if ( isImg2Img(settings) ){ img += 1; }
                else { txt += 1; }
            }
        })
        return {txt, img};
    ...
    return (
        <Flex m={3} direction={"column"} flex={1}>
            <Flex overflow={"auto"}>
                <Box flex='6'>
                    <MultiEditor
                        label={""}
                ...
                <SimpleGrid flex='1' columns={1} spacing={1}>
                    <Button onClick={() => enqueue(false)} flex='3'colorScheme='blue'>Generate</Button>
                    <Button onClick={() => enqueue(true)} flex='1' >Gnr with Rnd seed</Button>
                </SimpleGrid>
            </Flex>
            <Flex h={260}>
                <Box mr={10}>
                    <MultiEditor
                        ...
                        tools={
                            (_: VEP<string>) => <IconButton aria-label={"trash"} icon={<FaEraser/>}
                                ml={1}
                                onClick={() => updateTheStore(s => s.nextItems.ordered.forEach(id => {
                                    const item = s.nextItems.byId[id];
                                    if (isX2Img(item)) {
                                        item.seed = '';
                                    }
                                }))}/>}
